...
function PrimaryNav() {
  ...
  return (
    <nav
      ...
      ${bgColor ? 'bg-white/90 backdrop-blur-md shadow-lg border-b border-gray-200/50' : 'backdrop-blur-md shadow-lg border-b border-gray-800/30'}
    `}
      style={{
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 lg:h-20">
          ...
          <div className="flex-shrink-0 cursor-pointer group">
            <img src={logo} alt="Wakeel App Logo" className="h-8 lg:h-10 w-auto transform group-hover:scale-105 transition-transform duration-300" onClick={() => navigate('/')} />
          </div>
          ...
          {/* Mobile Menu Button */}
          <button className="lg:hidden relative w-10 h-10 flex flex-col justify-center items-center space-y-1 group" onClick={() => setToggle(!toggle)}>
            <span
              className={`
      ...
      <div
        ...
        ${toggle ? 'opacity-100 translate-y-0 max-h-screen' : 'opacity-0 -translate-y-4 max-h-0 pointer-events-none'}
      `}
      >
